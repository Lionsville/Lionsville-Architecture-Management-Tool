...
describe('canPlaceKind', () => {
  ...
  it('refuses every kind on a laid-out view, which holds no members at all', () => {
    for (const kind of ['application', 'actor', 'step', 'function', 'platform', 'platformService'] as const) {
  ...
  it('lets a platform onto a board, because the management band always drew one (ADR-0013)', () => {
    ...
    // And the service it realises, which is the same chip (ADR-0014).
    expect(canPlaceKind('platformService', 'layer7')).toEqual({ ok: true });
    expect(canPlaceKind('platformService', 'container')).toEqual({ ok: true });
  });
