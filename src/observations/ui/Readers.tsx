 ...
 *
 * The header — number, title, date, where, impact, seen, shared — is fields,
 * not text, drawn above the body so it cannot drift from it. The body is
 * markdown through the same renderer as documentation. The history at the end
 * is the dated ledger: recorded, seen again, shared, absorbed, merged.
 *
...
export function ObservationReader(props: ObservationReaderProps) {
  ...
  const canEdit = !readOnly && !fromScope && !mergedInto
  const stored = useMemo(() => ({
    title: observation.title, body: observation.body, where: observation.where ?? '', date: observation.date, impact: observation.impact,
  }), [observation])
                  ...
                  : (
                    <LinkList
                      ...
                      links={props.explainedBy.map(({ cause, link }) => ({
                        ...
                        ...(readOnly || mergedInto ? {} : { onRemove: () => props.onUnlink(cause.id), removeLabel: s('observation.unlink') }),
                      }))}
                    />
                  )}
              </Value>
