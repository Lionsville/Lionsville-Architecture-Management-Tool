...
describe('the stakeholder rail', () => {
  it('is the actor tree in order, outside parties marked, a party of a party one step in', () => {
  ...
  it('is empty when the sheet says the rail is off, and drawn when it says nothing', () => {
...
describe('the journey band', () => {
  it('reads the phases across the top, the common path first, then a row per lane', () => {
  ...
  it('marks a step somebody outside does, and leaves the common path unmarked', () => {
  ...
  it('has no band when the sheet names no journey, or one this scope does not hold', () => {
...
describe('the areas', () => {
  it('draws the roots the sheet names in its order, and the model’s own where it says nothing', () => {
  ...
  it('carries the domain an area is assigned to, and leaves out one the scope does not hold', () => {
    const { elements, relations } = shippingScope()
...
describe('the unmapped band', () => {
  it('holds the roots nobody placed, and nothing once every root is drawn', () => {
    const { elements, relations } = shippingScope()
    ...
    // One that has been given a domain is not in the band — that is what the
    // band is about.
    const placed = { elements: [...elements, area('compliance', 'Compliance', { scopes: ['risk'] })], relations }
    expect(sheetPage(placed, SHEET).unmapped).toEqual([])
